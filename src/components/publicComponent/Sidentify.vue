...
export default {
  ...
  data() {
    return {
      ...
      contentWidth: 130,
      contentHeight: 35,
      identifyCode: "abcdefghijklmnopqrstuvwxyz1234567890",
      newstr: [],
    };
  },
  methods: {
    ...
    drawPic() {
      ...
      for (let i = 0; i < 4; i++) {
        this.newstr.push(this.identifyCode[this.randomNum(36, 0)]);
      }
      for (let i = 0; i < this.newstr.length; i++) {
        this.drawText(ctx, this.newstr[i], i);
      }
    ...
    drawText(ctx, txt, i) {
      ...
      ctx.textBaseline = "bottom"; //基线对齐
      let x = (i + 1) * (this.contentWidth / (4 + 1));
      let y = this.randomNum(this.fontSizeMax, this.contentHeight - 5);
      var deg = this.randomNum(-30, 35);
      // 修改坐标原点和旋转角度
